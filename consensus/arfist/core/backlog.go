...
func (c *core) checkMessage(msgCode uint64, view *arfist.View) error {
...
func (c *core) storeBacklog(msg *message, src arfist.Validator) {
...
func (c *core) processBacklog() {
	...
	for src, backlog := range c.backlogs {
		...
		for !(backlog.Empty() || isFuture) {
			m, prio := backlog.Pop()
			msg := m.(*message)
			var view *arfist.View
			switch msg.Code {
			case msgPreprepare:
				var m *arfist.Preprepare
				err := msg.Decode(&m)
				if err == nil {
					view = m.View
				}
...
func toPriority(msgCode uint64, view *arfist.View) float32 {
	if msgCode == msgRoundChange {
