...
// New creates an ArfIst consensus core
func New(backend arfist.Backend, config *arfist.Config) Engine {
	c := &core{
		config:             config,
		...
		backend:            backend,
		backlogs:           make(map[arfist.Validator]*prque.Prque),
		backlogsMu:         new(sync.Mutex),
		pendingRequests:    prque.New(),
		pendingRequestsMu:  new(sync.Mutex),
		consensusTimestamp: time.Time{},
		roundMeter:         metrics.NewRegisteredMeter("consensus/arfist/core/round", nil),
		sequenceMeter:      metrics.NewRegisteredMeter("consensus/arfist/core/sequence", nil),
		consensusTimer:     metrics.NewRegisteredTimer("consensus/arfist/core/consensus", nil),
	}
...
type core struct {
	...
	backend               arfist.Backend
	events                *event.TypeMuxSubscription
	finalCommittedSub     *event.TypeMuxSubscription
	timeoutSub            *event.TypeMuxSubscription
	futurePreprepareTimer *time.Timer

	valSet                arfist.ValidatorSet
	waitingForRoundChange bool
	validateFn            func([]byte, []byte) (common.Address, error)

	backlogs   map[arfist.Validator]*prque.Prque
...
func (c *core) currentView() *arfist.View {
	return &arfist.View{
...
func (c *core) commit() {
	...
	if proposal != nil {
		...
		for i, v := range c.current.Commits.Values() {
			committedSeals[i] = make([]byte, types.ArfIstExtraSeal)
...
func (c *core) catchUpRound(view *arfist.View) {
...
func (c *core) updateRoundState(view *arfist.View, validatorSet arfist.ValidatorSet, roundChange bool) {
...
func (c *core) checkValidatorSignature(data []byte, sig []byte) (common.Address, error) {
	return arfist.CheckValidatorSignature(c.valSet, data, sig)
}
