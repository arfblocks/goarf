...
func newRoundState(view *arfist.View, validatorSet arfist.ValidatorSet, lockedHash common.Hash, preprepare *arfist.Preprepare, pendingRequest *arfist.Request, hasBadProposal func(hash common.Hash) bool) *roundState {
	return &roundState{
		round:          view.Round,
...
type roundState struct {
	round          *big.Int
	sequence       *big.Int
	Preprepare     *arfist.Preprepare
	Prepares       *messageSet
	Commits        *messageSet
	lockedHash     common.Hash
	pendingRequest *arfist.Request
...
func (s *roundState) Subject() *arfist.Subject {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *roundState) SetPreprepare(preprepare *arfist.Preprepare) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *roundState) Proposal() arfist.Proposal {
	s.mu.RLock()
	defer s.mu.RUnlock()
