...
import (
	...
	elog "github.com/tayfunakcay/goArf/log"
)
...
type testCommittedMsgs struct {
	commitProposal arfist.Proposal
	committedSeals [][]byte
}

// ==============================================
//
// define the functions that needs to be provided for ArfIst.
...
func (self *testSystemBackend) Validators(proposal arfist.Proposal) arfist.ValidatorSet {
	return self.peers
}
...
func (self *testSystemBackend) Send(message []byte, target common.Address) error {
	...
	self.sys.queuedMessage <- arfist.MessageEvent{
		Payload: message,
	}
	return nil
}

func (self *testSystemBackend) Broadcast(valSet arfist.ValidatorSet, message []byte) error {
	testLogger.Info("enqueuing a message...", "address", self.Address())
	self.sentMsgs = append(self.sentMsgs, message)
	self.sys.queuedMessage <- arfist.MessageEvent{
		Payload: message,
...
func (self *testSystemBackend) Gossip(valSet arfist.ValidatorSet, message []byte) error {
	testLogger.Warn("not sign any data")
	return nil
}

func (self *testSystemBackend) Commit(proposal arfist.Proposal, seals [][]byte) error {
	...
	go self.events.Post(arfist.FinalCommittedEvent{})
	return nil
}

func (self *testSystemBackend) Verify(proposal arfist.Proposal) (time.Duration, error) {
	return 0, nil
}
...
func (self *testSystemBackend) NewRequest(request arfist.Proposal) {
	go self.events.Post(arfist.RequestEvent{
		Proposal: request,
	})
}
...
func (self *testSystemBackend) LastProposal() (arfist.Proposal, common.Address) {
...
func (self *testSystemBackend) ParentValidators(proposal arfist.Proposal) arfist.ValidatorSet {
	return self.peers
}
...
type testSystem struct {
	...
	queuedMessage chan arfist.MessageEvent
	quit          chan struct{}
}
...
func newTestSystem(n uint64) *testSystem {
	...
	return &testSystem{
		...
		queuedMessage: make(chan arfist.MessageEvent),
...
func newTestValidatorSet(n int) arfist.ValidatorSet {
	return validator.NewSet(generateValidators(n), arfist.RoundRobin)
}
...
func NewTestSystemWithBackend(n, f uint64) *testSystem {
	...
	sys := newTestSystem(n)
	config := arfist.DefaultConfig

	for i := uint64(0); i < n; i++ {
		vset := validator.NewSet(addrs, arfist.RoundRobin)
		...
		core.current = newRoundState(&arfist.View{
			Round:    big.NewInt(0),
...
func (t *testSystem) Run(core bool) func() {
	for _, b := range t.backends {
		if core {
			b.engine.Start() // start ArfIst core
		}
	}
