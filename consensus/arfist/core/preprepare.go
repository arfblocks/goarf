...
func (c *core) sendPreprepare(request *arfist.Request) {
	...
	if c.current.Sequence().Cmp(request.Proposal.Number()) == 0 && c.isProposer() {
		...
		preprepare, err := Encode(&arfist.Preprepare{
			View:     curView,
...
func (c *core) handlePreprepare(msg *message, src arfist.Validator) error {
	logger := c.logger.New("from", src, "state", c.state)

	// Decode PRE-PREPARE
	var preprepare *arfist.Preprepare
...
func (c *core) acceptPreprepare(preprepare *arfist.Preprepare) {
