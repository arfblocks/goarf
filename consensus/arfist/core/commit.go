...
func (c *core) sendCommitForOldBlock(view *arfist.View, digest common.Hash) {
	sub := &arfist.Subject{
		View:   view,
		Digest: digest,
	}
...
func (c *core) broadcastCommit(sub *arfist.Subject) {
...
func (c *core) handleCommit(msg *message, src arfist.Validator) error {
	// Decode COMMIT message
	var commit *arfist.Subject
...
func (c *core) verifyCommit(commit *arfist.Subject, src arfist.Validator) error {
...
func (c *core) acceptCommit(msg *message, src arfist.Validator) error {
	logger := c.logger.New("from", src, "state", c.state)
