...
func TestHandlePreprepare(t *testing.T) {
	...
	testCases := []struct {
		system          *testSystem
		expectedRequest arfist.Proposal
		expectedErr     error
		existingBlock   bool
	}{
		{
...
func TestHandlePreprepareWithLock(t *testing.T) {
	...
	testCases := []struct {
		system       *testSystem
		proposal     arfist.Proposal
		lockProposal arfist.Proposal
	}{
		{
			newSystem(),
			proposal,
	...
	for _, test := range testCases {
		...
		lockPreprepare := &arfist.Preprepare{
			View:     curView,
