...
func testPreprepare(t *testing.T) {
	...
	var decodedPP *arfist.Preprepare
	err = decodedMsg.Decode(&decodedPP)
	if err != nil {
		t.Errorf("error mismatch: have %v, want nil", err)
	}

	// if block is encoded/decoded by rlp, we cannot to compare interface data type using reflect.DeepEqual. (like arfist.Proposal)
...
func testSubjectWithSignature(t *testing.T) {
	...
	decodedMsg = new(message)
	err = decodedMsg.FromPayload(msgPayload, func(data []byte, sig []byte) (common.Address, error) {
		return common.Address{}, arfist.ErrUnauthorizedAddress
	})
	if err != arfist.ErrUnauthorizedAddress {
		t.Errorf("error mismatch: have %v, want %v", err, arfist.ErrUnauthorizedAddress)
	}
}
