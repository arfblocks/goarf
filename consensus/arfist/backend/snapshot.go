...
const (
	dbKeySnapshotPrefix = "arfist-snapshot"
)
...
type Snapshot struct {
	...
	ValSet arfist.ValidatorSet    // Set of authorized validators at this moment
}

// newSnapshot create a new snapshot with the specified startup parameters. This
// method does not initialize the set of recent validators, so only ever use if for
// the genesis block.
func newSnapshot(epoch uint64, number uint64, hash common.Hash, valSet arfist.ValidatorSet) *Snapshot {
	snap := &Snapshot{
		Epoch:  epoch,
		Number: number,
		Hash:   hash,
...
type snapshotJSON struct {
	...
	Policy     arfist.ProposerPolicy `json:"policy"`
}
