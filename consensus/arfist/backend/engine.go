...
import (
	...
	arfistCore "github.com/tayfunakcay/goArf/consensus/arfist/core"
...
var (
	...
	// errInvalidMixDigest is returned if a block's mix digest is not ArfIst digest.
	errInvalidMixDigest = errors.New("invalid ArfIst mix digest")
...
func (sb *backend) verifyHeader(chain consensus.ChainReader, header *types.Header, parents []*types.Header) error {
	...
	if _, err := types.ExtractArfIstExtra(header); err != nil {
	...
	if header.MixDigest != types.ArfIstDigest {
		return errInvalidMixDigest
	}
	// Ensure that the block doesn't contain any uncles which are meaningless in ArfIst
...
func (sb *backend) verifyCommittedSeals(chain consensus.ChainReader, header *types.Header, parents []*types.Header) error {
	...
	extra, err := types.ExtractArfIstExtra(header)
	if err != nil {
		return err
	}
	...
	validSeal := 0
	proposalSeal := arfistCore.PrepareCommittedSeal(header.Hash())
	// 1. Get committed seals from current header
	for _, seal := range extra.CommittedSeal {
		// 2. Get the original address by seal and parent block hash
		addr, err := arfist.GetSignatureAddress(proposalSeal, seal)
		if err != nil {
...
func (sb *backend) Prepare(chain consensus.ChainReader, header *types.Header) error {
	...
	header.MixDigest = types.ArfIstDigest
...
func (sb *backend) Finalize(chain consensus.ChainReader, header *types.Header, state *state.StateDB, txs []*types.Transaction,
	...
	// No block rewards in ArfIst, so the state remains as is and uncles are dropped
...
func (sb *backend) Seal(chain consensus.ChainReader, block *types.Block, stop <-chan struct{}) (*types.Block, error) {
	...
	// post block into ArfIst engine
	go sb.EventMux().Post(arfist.RequestEvent{
		Proposal: block,
	})
...
func (sb *backend) APIs(chain consensus.ChainReader) []rpc.API {
	return []rpc.API{{
		Namespace: "arfist",
		Version:   "1.0",
		Service:   &API{chain: chain, arfist: sb},
		Public:    true,
	}}
}

// Start implements consensus.ArfIst.Start
func (sb *backend) Start(chain consensus.ChainReader, currentBlock func() *types.Block, hasBadBlock func(hash common.Hash) bool) error {
	...
	if sb.coreStarted {
		return arfist.ErrStartedEngine
	}
...
// Stop implements consensus.ArfIst.Stop
...
func (sb *backend) snapshot(chain consensus.ChainReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
	...
	for snap == nil {
		...
		if number == 0 {
			...
			arfistExtra, err := types.ExtractArfIstExtra(genesis)
			if err != nil {
				return nil, err
			}
			snap = newSnapshot(sb.config.Epoch, 0, genesis.Hash(), validator.NewSet(arfistExtra.Validators, sb.config.ProposerPolicy))
...
// FIXME: Need to update this for ArfIst
// sigHash returns the hash which is used as input for the ArfIst
...
func sigHash(header *types.Header) (hash common.Hash) {
	...
	rlp.Encode(hasher, types.ArfIstFilteredHeader(header, false))
	hasher.Sum(hash[:0])
	return hash
}
...
func ecrecover(header *types.Header) (common.Address, error) {
	...
	arfistExtra, err := types.ExtractArfIstExtra(header)
	if err != nil {
		return common.Address{}, err
	}

	addr, err := arfist.GetSignatureAddress(sigHash(header).Bytes(), arfistExtra.Seal)
	if err != nil {
		return addr, err
	}
...
func prepareExtra(header *types.Header, vals []common.Address) ([]byte, error) {
	...
	// compensate the lack bytes if header.Extra is not enough ArfIstExtraVanity bytes.
	if len(header.Extra) < types.ArfIstExtraVanity {
		header.Extra = append(header.Extra, bytes.Repeat([]byte{0x00}, types.ArfIstExtraVanity-len(header.Extra))...)
	}
	buf.Write(header.Extra[:types.ArfIstExtraVanity])

	ist := &types.ArfIstExtra{
		Validators:    vals,
		Seal:          []byte{},
		CommittedSeal: [][]byte{},
	}
...
func writeSeal(h *types.Header, seal []byte) error {
	if len(seal)%types.ArfIstExtraSeal != 0 {
		return errInvalidSignature
	}

	arfistExtra, err := types.ExtractArfIstExtra(h)
	if err != nil {
		return err
	}

	arfistExtra.Seal = seal
	payload, err := rlp.EncodeToBytes(&arfistExtra)
	if err != nil {
		return err
	}

	h.Extra = append(h.Extra[:types.ArfIstExtraVanity], payload...)
	return nil
}
...
func writeCommittedSeals(h *types.Header, committedSeals [][]byte) error {
	...
	for _, seal := range committedSeals {
		if len(seal) != types.ArfIstExtraSeal {
			return errInvalidCommittedSeals
		}
	}
	...
	arfistExtra.CommittedSeal = make([][]byte, len(committedSeals))
	copy(arfistExtra.CommittedSeal, committedSeals)
