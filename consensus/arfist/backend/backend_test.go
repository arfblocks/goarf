...
func TestCheckValidatorSignature(t *testing.T) {
	...
	for i, k := range keys {
		...
		addr, err := arfist.CheckValidatorSignature(vset, data, sig)
		if err != nil {
			t.Errorf("error mismatch: have %v, want nil", err)
		}
...
func TestCommit(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			// normal case
			nil,
			[][]byte{append([]byte{1}, bytes.Repeat([]byte{0x00}, types.ArfIstExtraSeal-1)...)},
			func() *types.Block {
...
func newTestValidatorSet(n int) (arfist.ValidatorSet, []*ecdsa.PrivateKey) {
	// generate validators
	keys := make(Keys, n)
