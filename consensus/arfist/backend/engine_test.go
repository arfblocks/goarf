...
// in this test, we can set n to 1, and it means we can process ArfIst and commit a
// block by one node. Otherwise, if n is larger than 1, we have to generate
// other fake events to process ArfIst.
...
func getGenesisAndKeys(n int) (*core.Genesis, []*ecdsa.PrivateKey) {
	...
	// force enable ArfIst engine
	genesis.Config.ArfIst = &params.ArfIstConfig{}
	genesis.Config.Ethash = nil
	genesis.Difficulty = defaultDifficulty
	genesis.Nonce = emptyNonce.Uint64()
	genesis.Mixhash = types.ArfIstDigest
...
func appendValidators(genesis *core.Genesis, addrs []common.Address) {
	...
	if len(genesis.ExtraData) < types.ArfIstExtraVanity {
		genesis.ExtraData = append(genesis.ExtraData, bytes.Repeat([]byte{0x00}, types.ArfIstExtraVanity)...)
	}
	genesis.ExtraData = genesis.ExtraData[:types.ArfIstExtraVanity]
	...
	if err != nil {
		panic("failed to encode arfist extra")
	}
	genesis.ExtraData = append(genesis.ExtraData, istPayload...)
}

func makeHeader(parent *types.Block, config *arfist.Config) *types.Header {
	header := &types.Header{
...
func TestSealStopChannel(t *testing.T) {
	...
	eventSub := engine.EventMux().Subscribe(arfist.RequestEvent{})
	eventLoop := func() {
		ev := <-eventSub.Chan()
		_, ok := ev.Data.(arfist.RequestEvent)
		if !ok {
...
func TestSealCommittedOtherHash(t *testing.T) {
	...
	<-timeout.C
	// wait 2 seconds to ensure we cannot get any blocks from ArfIst
}
...
func TestPrepareExtra(t *testing.T) {
	...
	vanity := make([]byte, types.ArfIstExtraVanity)
...
func TestWriteSeal(t *testing.T) {
	vanity := bytes.Repeat([]byte{0x00}, types.ArfIstExtraVanity)
	...
	expectedSeal := append([]byte{1, 2, 3}, bytes.Repeat([]byte{0x00}, types.ArfIstExtraSeal-3)...)
	expectedIstExtra := &types.ArfIstExtra{
	...
	// verify arfist extra-data
	istExtra, err := types.ExtractArfIstExtra(h)
	if err != nil {
		t.Errorf("error mismatch: have %v, want nil", err)
	}
...
func TestWriteCommittedSeals(t *testing.T) {
	vanity := bytes.Repeat([]byte{0x00}, types.ArfIstExtraVanity)
	istRawData := hexutil.MustDecode("0xf858f8549444add0ec310f115a0e603b2d7db9f067778eaf8a94294fc7e8f22b3bcdcf955dd7ff3ba2ed833f8212946beaaed781d2d2ab6350f5c4566a2c6eaac407a6948be76812f765c24641ec63dc2852b378aba2b44080c0")
	expectedCommittedSeal := append([]byte{1, 2, 3}, bytes.Repeat([]byte{0x00}, types.ArfIstExtraSeal-3)...)
	expectedIstExtra := &types.ArfIstExtra{
		Validators: []common.Address{
			common.BytesToAddress(hexutil.MustDecode("0x44add0ec310f115a0e603b2d7db9f067778eaf8a")),
			common.BytesToAddress(hexutil.MustDecode("0x294fc7e8f22b3bcdcf955dd7ff3ba2ed833f8212")),
