...
func TestHeaderHash(t *testing.T) {
	...
	// for arfist consensus
	header := &Header{MixDigest: ArfIstDigest, Extra: expectedExtra}
...
func TestExtractToArfIst(t *testing.T) {
	testCases := []struct {
		vanity         []byte
		istRawData     []byte
		expectedResult *ArfIstExtra
		expectedErr    error
	}{
		{
			// normal case
			bytes.Repeat([]byte{0x00}, ArfIstExtraVanity),
		...
		{
			// insufficient vanity
			bytes.Repeat([]byte{0x00}, ArfIstExtraVanity-1),
			nil,
			nil,
			ErrInvalidArfIstHeaderExtra,
		},
	}
	for _, test := range testCases {
		h := &Header{Extra: append(test.vanity, test.istRawData...)}
		arfistExtra, err := ExtractArfIstExtra(h)
		if err != test.expectedErr {
			t.Errorf("expected: %v, but got: %v", test.expectedErr, err)
		}
		if !reflect.DeepEqual(arfistExtra, test.expectedResult) {
			t.Errorf("expected: %v, but got: %v", test.expectedResult, arfistExtra)
		}
	}
}
